...
export default {
  ...
  methods: {
    pathAnimate(result, tempSphere) {
      ...
      if (this.timesRun == result.length) {
        return;
      }

      const next = {
        x: result[this.timesRun].y * 10 - length / 2 + 5,
        z: result[this.timesRun].x * 10 - length / 2 + 5,
      };
      console.log(this.timesRun);
      if (tempSphere.position.x == next.x && tempSphere.position.z == next.z) {
        requestAnimationFrame(this.pathAnimate(result, tempSphere));
        return;
      }
    ...
    pa(result, tempSphere, nextP = 0) {
      ...
      const animationTimer = setInterval(() => {
        // FIXME
        if (nextP === result.length) {
          clearInterval(animationTimer);
        }
        const nextPath = {
          x: result[nextP].y * 10 - length / 2 + 5,
          z: result[nextP].x * 10 - length / 2 + 5,
        };
        console.log(nextPath, tempSphere.position.x);
        if (tempSphere.position.x == nextPath.x && tempSphere.position.z == nextPath.z) {
          nextP++;

          return;
        }
        if (tempSphere.position.x > nextPath.x) tempSphere.position.x -= step;
        if (tempSphere.position.x < nextPath.x) tempSphere.position.x += step;
        if (tempSphere.position.z > nextPath.z) tempSphere.position.z -= step;
        if (tempSphere.position.z < nextPath.z) tempSphere.position.z += step;
      }, interval);
    },
    ...
    caculatePath(resultArray) {
      const length = this.length;
      const maps = new Graph(this.graph);

      const startX = (resultArray[0].position.z - 5 + length / 2) / 10;
      const startY = (resultArray[0].position.x - 5 + length / 2) / 10;
      const endX = (resultArray[1].position.z - 5 + length / 2) / 10;
      const endY = (resultArray[1].position.x - 5 + length / 2) / 10;
      ...
      if (result.length === 0) {
        this.$message.error('无可到达路径');
        this.cleanSphere();
        return;
      }
      this.pa(result, resultArray[0]);
    },
    rayClick(e) {
      const length = this.length;
